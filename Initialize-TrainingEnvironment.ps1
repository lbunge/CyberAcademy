<#
.Synopsis
   PowerShell script to build out a training environment in Microsoft Azure for a hardening exercise
.DESCRIPTION
   https://techcommunity.microsoft.com/t5/fasttrack-for-azure/deploying-apache-guacamole-on-azure/ba-p/3269613
.EXAMPLE
#>

function New-TrainingRange {
   [CmdletBinding()]
   Param (
        $resourceGroupName = "testing2",
        $resourceGroupLocation = "eastus",
        $mysqldbName = 'testingdb' + (Get-Random),
        ...
        $domainName = "lab.glrcyberacademy.com",
        $email = "admin@glrcyberacademy.com"
   )
   Begin {
      ...
      Process {
        ...
        New-AzMySQLServer -Name $mysqldbName -resourceGroupName $resourceGroupName -Location $resourceGroupLocation `
            -AdministratorUserName $mysqladmin `
            -AdministratorLoginPassword $securemysqlpassword `
            -Sku "B_Gen5_1" `
            -StorageInMb 51200 `
            -SslEnforcement "Disabled" #TODO: Try out Enabling SSL
        
        
        # Create Firewall Rule for the SQL Server
        New-AzMySqlFirewallRule -ServerName $mysqldbName -ResourceGroupName $resourceGroupName `
            -Name "AllowYourIP" `
            -StartIPAddress 0.0.0.0 `
            -EndIPAddress 255.255.255.255 #TODO: Try out smaller IP range
        ...
        # Create the NSG Rule for HTTPS Traffic
        ...
        # Create Inbound NAT Rule for VM01 SSH Traffic
        $inatRule1 = New-AzLoadBalancerInboundNatRuleConfig `
            -Name "inatRule-SSH1" `
        ...
        # Create Inbound NAT Rule for VM02 SSH Traffic
        $inatRule2 = New-AzLoadBalancerInboundNatRuleConfig `
            -Name "inatRule-SSH2" `
            -FrontendIPConfiguration $frontend `
            -Protocol "Tcp" `
            -FrontendPort 23 `
        ...
        $loadbalancer = New-AzLoadBalancer -ResourceGroupName $resourceGroupName -Location $resourceGroupLocation `
            ...
            -FrontendIPConfiguration $frontend `
            -BackendAddressPool $backendAddressPool `
            -Probe $healthprobeHTTP,$healthprobeHTTPS `
            -InboundNatRule $inatRule1,$inatRule2 `
