...
# Configure SSL - Comment out this section if you wish to test without SSL
sed -i.bkp -e "s/_;/$domainName;/g" /etc/nginx/sites-enabled/default

## Install snap tool and certbot
snap install core; snap refresh core
snap install --classic certbot
certbot --nginx -d "$domainName" -m "$email" --agree-tos -n

## Restart Nginx
systemctl restart nginx

## Create Cron Job to automatically renew certificates
...
# Install branding extention
wget https://raw.githubusercontent.com/lbunge/CyberAcademy/main/branding.jar -O branding.jar
